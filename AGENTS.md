...
- `pnpm deploy` (`wrangler deploy`) still works and pushes the local `dist/` straight to the Worker, bypassing the repo. Use it only for an out-of-band deploy, and rebuild first, since it ships whatever `dist/` currently holds.
