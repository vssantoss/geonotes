...
- [~] Native bot resistance (Android implemented; deploy + on-device test remain). Full rationale in the "Native bot resistance" part of the moved section below:
  ...
  - [ ] DEPLOY: set the Worker secret `PLAY_INTEGRITY_SA_JSON` from `~/dev/geonotes-vshub-b53779f9aea5.json` before/with the push, else native `email-request` falls back to Turnstile (which the app can't solve) and 403s. Leave `PLAY_INTEGRITY_STRICT` unset until the app ships from a Play track.
