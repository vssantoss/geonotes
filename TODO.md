...
- ~~Set up ESLint.~~ **DONE (2026-07-19, branch `eslint-setup`).** ESLint 10 + typescript-eslint 8.64 with the type-checked rule set, plus react-hooks 7 and react-refresh on the frontend; `pnpm lint` added and documented in AGENTS.md.
  ...
  - Left behind: **three react-hooks 7 findings in existing frontend code are downgraded to warnings** so they do not block every lint run. Tracked as its own item below.
...
- Fix the three react-hooks findings that are currently downgraded to warnings. `eslint.config.js` sets `react-hooks/set-state-in-effect` and `react-hooks/refs` to `warn` rather than `error`, purely so the pre-existing violations below do not block every lint run. They are real findings, not false positives, and the downgrade should be reverted once they are fixed. Each wants a different fix, so they can be done independently:
  - `src/screens/EditorScreen.tsx:60` writes `textRef.current = text` during render. **The most worth fixing of the three:** writing a ref during render is a genuine React correctness smell, not a style preference, and it can leave the ref stale or torn under concurrent rendering. The ref exists to give the focus listener the latest text without re-subscribing on every keystroke, so the fix is to move the write into an effect (or drop the ref for a different way of reading current text in the listener).
  - `src/components/settings/PasskeysSection.tsx:39` and `src/components/settings/SessionsSection.tsx:45` both call `void reload()` inside a mount-only `useEffect`, which setStates synchronously and triggers a cascading render. This is the common "load on mount" pattern and the rule is arguably over-strict about it, but both also carry an `exhaustive-deps` warning for the missing `reload` dependency, which is the more real of the two problems. Fixing the data loading properly (or wrapping `reload` in `useCallback` and depending on it) clears both at once.
